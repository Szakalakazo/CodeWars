package Lvl_6;

public class MainLvl6 {
    public static void main(String[] args) {
        int[] array = new int[]{20, 1, -1, 2, -2, 3, 3, 5, 5, 1, 2, 4, 20, 4, -1, -2, 5};
        int[] array2 = new int[]{2, 4, 0, 100, 4, 11, 2602, 36};
        String stringArray[] = {"Hi", "Hello", "Howdy", "Bye"};
        String text = "aabbcde";



        //Multiples_of_3_or_5
        Multiples_of_3_or_5.solution(10);

//        System.out.println(FindOdd.findIt(array2));
//        System.out.println(FindOutlier.find(array2));
//        System.out.println(ReverseWords.spinWords("Hey fellow warriors"));
//        System.out.println(MorseCodeDecoder.decode(".... . -.--   .--- ..- -.. ."));
//        YourOrder.order("is2 Thi1s T4est 3a");
//        System.out.println(Dubstep.SongDecoder("JKD WBIRAQKF YE WV"));
        System.out.println(CountTheDuplicate.duplicateCount(text));

    }
}
